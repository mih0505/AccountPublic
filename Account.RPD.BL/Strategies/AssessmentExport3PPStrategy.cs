...
{
    public class AssessmentExport3PPStrategy : IAssessmentExportStrategy
    {
        ...
        public void Include(IDictionary<DecanatCompetence, IEnumerable<CompetenceGrade>> competenceGradesDictionary, IDictionary<DecanatCompetence, Competence> competences)
        {
            ...
            var competenceGradesTable = document.AddTable(3 + competenceGradesDictionary.Keys.Count * 3, 8);
            ...
            competenceGradesTable.Rows[0].Cells[1].Paragraphs[0].Append("Код и наименование индикатора достижения компетенции")
                                                                .Bold(isBold: true)
                                                                .Alignment = Alignment.center;
            ...
            competenceGradesTable.Rows[0].Cells[2].Paragraphs[0].Append("Результаты обучения по дисциплине (модулю)")
                                                                .Bold(isBold: true)
                                                                .Alignment = Alignment.center;
            ...
            competenceGradesTable.Rows[0].MergeCells(startIndex: 3, endIndex: 6);
            competenceGradesTable.Rows[0].Cells[3].Paragraphs[0].Append("Показатели и критерии оценивания результатов обучения по дисциплине (модулю)")
                                                                .Bold(isBold: true)
                                                                .Alignment = Alignment.center;

            competenceGradesTable.Rows[1].MergeCells(startIndex: 3, endIndex: 6);
            competenceGradesTable.Rows[1].Cells[3].Paragraphs[0].Append("4")
                                                                .Bold(isBold: true)
                                                                .Alignment = Alignment.center;

            competenceGradesTable.Rows[2].Cells[3].Paragraphs[0].Append("неуд.")
                                                                .Bold(isBold: true)
                                                                .Alignment = Alignment.center;

            competenceGradesTable.Rows[2].Cells[4].Paragraphs[0].Append("удовл.")
                                                                .Bold(isBold: true)
                                                                .Alignment = Alignment.center;

            competenceGradesTable.Rows[2].Cells[5].Paragraphs[0].Append("хорошо")
                                                                .Bold(isBold: true)
                                                                .Alignment = Alignment.center;

            competenceGradesTable.Rows[2].Cells[6].Paragraphs[0].Append("отлично")
                                                                .Bold(isBold: true)
                                                                .Alignment = Alignment.center;

            competenceGradesTable.Rows[0].Cells[4].Paragraphs[0].Append("Вид оценочного средства")
                                                                .Bold(isBold: true)
                                                                .Alignment = Alignment.center;

            competenceGradesTable.Rows[1].Cells[4].Paragraphs[0].Append("5")
                                                                .Bold(isBold: true)
                                                                .Alignment = Alignment.center;
            ...
            foreach (var keyValuePair in competenceGradesDictionary)
            {
                ...
                competenceGradesTable.Rows[index].Cells[0].Paragraphs[0].Append($"{keyValuePair.Key.Code}. {keyValuePair.Key.Content}");

                var subIndex = index;
                var stageCounter = 1;
                foreach (var competenceStage in keyValuePair.Value)
                {
                    var competence = competences.FirstOrDefault(KeyValuePair => KeyValuePair.Key.Id.Equals(competenceStage.DecanatId)).Value;
                    var stageName = string.Empty;

                    if (stageCounter.Equals(1))
                    {
                        stageName = competence.KnowledgeName;
                    }
                    else if (stageCounter.Equals(2))
                    {
                        stageName = competence.SkillName;
                    }
                    else if (stageCounter.Equals(3))
                    {
                        stageName = competence.PossessionName;
                    }

                    competenceGradesTable.Rows[subIndex].Cells[1].Paragraphs[0].Append($"{competenceStage.Stage}. {stageName}");

                    competenceGradesTable.Rows[subIndex].Cells[3].Paragraphs[0].Append(competenceStage.BadGrade);

                    competenceGradesTable.Rows[subIndex].Cells[4].Paragraphs[0].Append(competenceStage.TernGrade);

                    competenceGradesTable.Rows[subIndex].Cells[5].Paragraphs[0].Append(competenceStage.WellGrade);

                    competenceGradesTable.Rows[subIndex].Cells[6].Paragraphs[0].Append(competenceStage.PerfectGrade);

                    competenceGradesTable.Rows[subIndex].Cells[7].Paragraphs[0].Append(competenceStage.ValuationType);
            ...
            {
                competenceGradesTable.Rows[index++].Cells[2].Paragraphs[0].Append($"Обучающийся должен:\n{keyValuePair.Value.Knowledge}");
