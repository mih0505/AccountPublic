...
{
    public class RPDExport3PPStrategy : IRPDExportStrategy
    {
        ...
        public void Include(RPD rpd, DecanatDepartment decanatDepartment, IEnumerable<Member> members)
        {
            ...
            disciplineTable.Rows[0].Cells[1].Paragraphs[0].Append($"{rpd.Block} {rpd.DisciplineName}")
                                                          .Italic(isItalic: true)
                                                          .Bold(isBold: true)
            ...
            var blockTable = document.AddTable(1, 1);
            ...
            {
                part = "часть, формируемая участниками образовательных отношений";
            }
            // факультатив?

            blockTable.Rows[0].Cells[0].Paragraphs[0].Append(part)
                                                     .FontSize(10.0)
                                                     .Alignment = Alignment.center;
        ...
        public void Include(IDictionary<DecanatCompetence, Competence> competences)
        {
            ...
            var competenceRPDItem = rpdItems.FirstOrDefault(RPDItem => RPDItem.Number.Equals("1"));
            ...
            competenceRPDItemTable.Rows[0].Cells[0].Paragraphs[0].Append("Формируемая компетенция (с указанием кода)")
                                                                 .Bold(isBold: true)
                                                                 .Alignment = Alignment.center;

            competenceRPDItemTable.Rows[0].Cells[1].Paragraphs[0].Append("Код и наименование индикатора достижения компетенции")
                                                                 .Bold(isBold: true)
                                                                 .Alignment = Alignment.center;

            competenceRPDItemTable.Rows[0].Cells[2].Paragraphs[0].Append("Результаты обучения по дисциплине (модулю)")
                                                                 .Bold(isBold: true)
                                                                 .Alignment = Alignment.center;
            ...
            foreach (var keyValuePair in competences)
            {
                ...
                competenceRPDItemTable.Rows[index].Cells[0].Paragraphs[0].Append($"{keyValuePair.Key.Code}. {keyValuePair.Key.Content}");

                competenceRPDItemTable.Rows[index].Cells[1].Paragraphs[0].Append($"{keyValuePair.Key.Code}.1. {keyValuePair.Value.KnowledgeName}");
                competenceRPDItemTable.Rows[index].Cells[2].Paragraphs[0].Append($"Обучающийся должен:\n{keyValuePair.Value.Knowledge}");

                index++;

                competenceRPDItemTable.Rows[index].Cells[1].Paragraphs[0].Append($"{keyValuePair.Key.Code}.2. {keyValuePair.Value.SkillName}");
                competenceRPDItemTable.Rows[index].Cells[2].Paragraphs[0].Append($"Обучающийся должен:\n{keyValuePair.Value.Skill}");

                index++;

                competenceRPDItemTable.Rows[index].Cells[1].Paragraphs[0].Append($"{keyValuePair.Key.Code}.3. {keyValuePair.Value.PossessionName}");
        ...
        public void Include(DecanatPlan plan, IEnumerable<RPDContent> rpdContents, DecanatHoursDivision hoursDivision, IDictionary<string, string> formsOfControl, IEnumerable<int> courses, IEnumerable<int> semesters)
        {
            ...
            document.InsertParagraph("Цели изучения дисциплины:")
                    .AppendLine(disciplinePlaceContent?.Content)
                    .AppendLine()
